...
export default function DoorSensorCard({ label = 'Door Sensor' }: Props) {
  const [state, setState] = useState<string>('loading');
  const [ago, setAgo] = useState<string>('');

  const fetch_ = async () => {
    try {
      const res = await fetch('/api/states/binary_sensor.sensor_door_contact', { headers: HA_HEADERS });
      if (!res.ok) return;
  ...
  /* closed = off = good (teal); open = on = alert */
  return (
    <SensorCard
      label={label}
      sublabel={ago}
