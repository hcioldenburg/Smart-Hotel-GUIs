...
const To_be_deleted = ["light", "switch", "climate", "fan", "media_player", "cover", "binary_sensor.presencesensor_aqara_presence", "binary_sensor.sensor_window_contact", "binary_sensor.sensor_door_contact"];
...
export const fetchButtonAutomations = async (): Promise<Record<string, { id: string; name: string; entityId: string }>> => {
  const buttonAutomations = {
    ...
    'Roller Shutter Control Button (Button)': 'automation.ps_rolloswitchcontrol',
    'Button - Floor Lamp': 'automation.ps_standlighttoggle'
  };
...
const fetchAutomationConfigWS = (entityId: string): Promise<any | null> => {
  return new Promise((resolve, reject) => {
    ...
    ws.onmessage = (event) => {
      ...
      if (msg.type === "auth_ok") {
        ws.send(JSON.stringify({ id, type: `automation/config/${entityId}` }));
      }

      if (msg.type === "result" && msg.id === id) {
        ws.close();
        if (msg.success) resolve(msg.result);
        else resolve(null); // YAML-based automation
      }
